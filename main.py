...
# 공통 콜백: 버튼 클릭 시 session_state 업데이트
...
if st.session_state.step == 1:
    ...
    # 해당 단계에 대해 3개의 랜덤 이미지를 생성(이미 한 번 생성된 경우 그대로 사용)
    ...
    for i, col in enumerate(cols):
        with col:
            ...
            st.button(
                "이 이미지 선택",
                key=f"img_{st.session_state.image_step}_{i}",
                on_click=select_image,
                args=(st.session_state.current_images[i],)
            )
